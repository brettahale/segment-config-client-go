...
func (c *Client) ListCatalogSource(name string) ([]CatalogSource, error) {
    var source []CatalogSource
    log.Printf("[DEBUG] HTTP request to API %s ", fmt.Sprintf("%s/v1beta/catalog/sources/", c.HostURL))
	req, err := http.NewRequest("GET", fmt.Sprintf("%s/v1beta/catalog/sources/", c.HostURL), nil)
	if err != nil {
		return source, err
	}
...
func (c *Client) ListCatalogDestination() ([]CatalogDestination, error) {
    var destination []CatalogDestination
    log.Printf("[DEBUG] HTTP request to API %s ", fmt.Sprintf("%s/v1beta/catalog/destinations", c.HostURL))
	req, err := http.NewRequest("GET", fmt.Sprintf("%s/v1beta/catalog/destinations", c.HostURL), nil)
	if err != nil {
